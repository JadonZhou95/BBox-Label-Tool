...
class LabelTool():
    def __init__(self, master):
        ...
        self.img_size = ()
        self.max_size = (960, 960)  #define the max image size to be desplayed
    ...
    def loadImage(self):
        ...
        # resize the image if the src image is too large for labelling
        self.img_size = self.img.size
        self.getResizeFactor()
        self.img.thumbnail(self.max_size, Image.ANTIALIAS)
        # print("source image resolution:", self.img_size)
        # print("resize factor: {}".format(self.resize_factor))
        self.img_size = self.img.size
        # print("resized image resolution:", self.img_size)
        ...
        if os.path.exists(self.labelfilename):
            with open(self.labelfilename) as f:
                for (i, line) in enumerate(f):
                    ...
                    # consider the resize factor
                    tmpId = self.mainPanel.create_rectangle(int(int(tmp[0]) / self.resize_factor), \
                                                            int(int(tmp[1]) / self.resize_factor), \
                                                            int(int(tmp[2]) / self.resize_factor), \
                                                            int(int(tmp[3]) / self.resize_factor), \
                                                            width=2, \
                                                            outline=COLORS[(len(self.bboxList) - 1) % len(COLORS)])

                    # print tmpId
                    self.bboxIdList.append(tmpId)
                    # display the raw value of the bbox on the bar list
                    self.listbox.insert(END, '%s : (%d, %d) -> (%d, %d)' % (tmp[4], int(tmp[0]), int(tmp[1]), \
                                                                            int(tmp[2]), int(tmp[3])))
                    # draw the resized bbox on the displayed image
                    self.listbox.itemconfig(len(self.bboxIdList) - 1,
                                            fg=COLORS[(len(self.bboxIdList) - 1) % len(COLORS)])

    # compute the resize factor
    def getResizeFactor(self):
        if self.img_size[0] <= self.max_size[0] and self.img_size[1] <= self.max_size[1]:
            self.resize_factor = 1.0
        else:
            # self.resize_factor = max(self.img_size) / float(self.max_size[0])
            self.resize_factor = max(self.img_size[0] / float(self.max_size[0]), \
                                     self.img_size[1] / float(self.max_size[1]))
    ...
    def mouseClick(self, event):
        if self.STATE['click'] == 0:
            ...
            # resize the bbox info for display and save
